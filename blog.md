...
个人块: grid-column: 2 / span 1
路由块: grid-column: 3 / span 4
统计块: grid-column: 7 / span 1
容器最大宽度: 1600px
列间距: clamp(12px, 1.4vw, 24px)
页面水平留白: clamp(12px, 2vw, 32px)
```

个人块和统计块可以由多个连续区段组成，但区段自身不再套入另一张外层卡片。面板圆角控制在 8px 以内，使用半透明背景、细边框和克制的模糊效果。
...
| 范围 | 布局 |
|------|------|
| `>= 1180px` | 严格八列：空、个人、路由四列、统计、空 |
| `768px - 1179px` | 六列：左侧两列放个人和统计，中间路由占右侧四列 |
...
| `T + 0.08s` | 顶部栏 | 从 `yPercent: -110` 向下伸出，透明度同步恢复，持续约 0.68s |
| `T + 0.28s` | 个人块 | 从左侧 `x: -64`、轻微模糊和透明状态闪入，持续约 0.7s |
| `T + 0.28s` | 统计块 | 从右侧 `x: 64`、轻微模糊和透明状态闪入，持续约 0.7s |
| `T + 0.45s` | 路由块 | 以视频中伸出的手为锚点，被推向最终位置；同时 `opacity: 0 → 1`，持续约 0.85s |
...
- 中间路由块承载 Astro `<slot />`，文章列表、文章正文和其他页面共用同一框架。
- 首次进入站点播放完整水滴与入场动画。
...
1. 创建语义化 Astro 页面框架和八列/六列/单列响应式 Grid，不接动画。
...
- 桌面严格呈现八列结构，左右各空一列，个人/路由/统计占比为 1/4/1。
...
- [ ] 创建顶部栏与八列/六列/单列主页布局
- [ ] 创建个人块、统计块与中间路由块
- [ ] 添加 GSAP 统一入场时间线和视频手部锚点映射
- [ ] 将背景视频作为 VideoTexture 接入水面滤镜
